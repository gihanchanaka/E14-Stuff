...
greenSorted=sort(sixColours(:,3));
projectionSorted=sort(projectedM);

projectionSorted(:)=projectionSorted(:)-projectionSorted(1);
greenSorted(:)=greenSorted(:)-greenSorted(1);
figure;
hold on;
plot(greenSorted,'gd-');
plot(projectionSorted/1.5,'y*-');
hold off;